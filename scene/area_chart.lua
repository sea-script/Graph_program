...
function area_chart.load()
    ...
    areas = {}
    areas[1] = { x = display.x, y = display.y + display.height }
    areas[2] = { x = display.x + display.width, y = display.y + display.height }
...
function area_chart.mousepressed(mouse_x, mouse_y, mouseID)
    if mouseID == 1  then
        --drawing
        if mouse_x >= areas[#areas-1].x and (mouse_x >= display.x and mouse_x <= display.x + display.width and mouse_y >= display.y and mouse_y <= display.y + display.height ) then
                areas[#areas] = { x = mouse_x, y = mouse_y }
                areas[#areas + 1] = { x = display.x + display.width, y = display.y + display.height }   
        end
        --first button
...
function area_chart.draw()
    ...
    if #areas >= 3 then
        ...
        local points = {}
        for i = 1, #areas do
            table.insert(points, areas[i].x)
            table.insert(points, areas[i].y)
        end
